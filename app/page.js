...
export default function Home() {
  ...
  const handleShare = (e) => {
    ...
    const stringDescription = urlData
      .map((url) => `${url.description}`)
      .join(',')
    const stringChannelTitle = urlData
      .map((url) => `${url.channelTitle}`)
      .join(',')
    const stringPublishedAt = urlData
      .map((url) => `${new Date(url.publishedAt).toDateString()}`)
      .join(',')
